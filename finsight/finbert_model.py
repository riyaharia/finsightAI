...
# totalincomegrowth=data['insights']['totalIncomeGrowth']
...
if not os.path.exists("gemini_output.json"):
    print(json.dumps({"error": "gemini_output.json not found"}))
...
# ✅ Extract insights (handle missing keys safely)
insights_keys = ['netProfitGrowth', 'earningsPerShareGrowth', 'segmentPerformance', 'totalIncomeGrowth']
all_insights = []

for key in insights_keys:
    if key in data['insights']:  # Check if key exists
        all_insights += data['insights'][key]

# ✅ If no insights are found, return an error
if not all_insights:
    print(json.dumps({"error": "No valid insights found in JSON"}))
    exit(1)
...
# ✅ Only print valid JSON (no extra logs)
